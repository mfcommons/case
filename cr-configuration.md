...
Mobile Foundation Server is predefined with confidential clients for Admin Service. The credentials for these clients are provided in the `mfpserver.adminClientSecret`, `mfpserver.pushClientSecret` and `mfpserver.liveupdateClientSecret` fields. 
...
kubectl create secret generic mf-push-client --from-literal=MFPF_LIVEUPDATE_AUTH_CLIENTID=admin --from-literal=MFPF_LIVEUPDATE_AUTH_SECRET=admin
```
